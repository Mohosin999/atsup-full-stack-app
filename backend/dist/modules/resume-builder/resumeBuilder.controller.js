import { getAllResumesByUser, getResumeById, createResumeFromUpload, createResumeFromContent as createResumeFromContentService, updateResumeById, deleteResumeById, deleteAllResumesByUser, duplicateResumeById, } from "./subservices/resumes.service";
...
export const createResumeFromContent = async (req, res) => {
    try {
...
export const updateResume = async (req, res) => {
    try {
        const resume = await updateResumeById(req.params.id, req.user.id, req.body);
        if (!resume) {
            return res.status(404).json({
                success: false,
                message: "Resume not found",
