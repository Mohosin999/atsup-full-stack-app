...
export const getAllResumesByUser = async (userId, options) => {
...
export const createResumeFromContent = async (userId, content) => {
    ...
    const resume = await prisma.resume.create({
        data: {
            ...
            metadata: {
                ...
                originalName: content.personalInfo?.jobTitle ||
...
export const updateResumeById = async (resumeId, userId, updateData) => {
...
export const duplicateResumeById = async (resumeId, userId) => {
    const existing = await prisma.resume.findFirst({
        where: { id: resumeId, userId },
    });
    if (!existing) {
    ...
    const sourceTitle = existing.metadata?.originalName ||
        existing.content?.personalInfo?.jobTitle ||
        existing.content?.personalInfo?.fullName ||
        'Resume';
    const resume = await prisma.resume.create({
        data: {
            userId,
...
export const deleteAllResumesByUser = async (userId) => {
