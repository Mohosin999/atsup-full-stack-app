 ...
 */
export const matchDictionary = (text, dictionary) => {
    const haystack = text;
    const found = [];
    ...
    const filtered = found.filter((entry) => !found.some((other) => other !== entry &&
 ...
 */
export const countDictionaryMatches = (text, dictionary) => {
    const haystack = text;
    let total = 0;
    for (const group of dictionary) {
        if (!group || !group.length)
