...
// ============================================================================
import { HARD_SKILLS_DICTIONARY, HARD_SKILL_STOPWORDS, } from "../dictionaries/hard-skills.dictionary";
...
import { countWords, DATE_RANGE_RE, extractEmail, extractGithub, extractLinkedIn, extractMeasurableResults, extractPhone, extractPortfolio, NORMAL_DATE_RE, } from "../dictionaries/regex-helpers";
...
const isLocationLike = (role) => {
    const lower = role.toLowerCase();
    if (/^(freelance|contract|self[- ]employed|remote|contract|independent|part[- ]time|full[- ]time)/.test(lower))
        return true;
...
const sanitizeName = (name) => {
    ...
    if (/@/.test(trimmed) || /linkedin|github|http|www\./i.test(trimmed))
        return "";
    if (/^(summary|experience|education|skills|projects|certification|objective|profile)$/i.test(trimmed))
        return "";
    return trimmed;
};
// ============================================================================
...
const extractExperienceYears = (text) => {
    const m = text.match(/(?:^|\W)(\d+(?:\.\d+)?)\s*\+?\s*(?:years?|yrs?)(?:\W|$)/i);
...
const calculateExperienceYears = (experiences) => {
    ...
    for (let i = 1; i < allDates.length; i++) {
        if (allDates[i].start <= currentRange.end) {
            ...
            currentRange.end = new Date(Math.max(currentRange.end.getTime(), allDates[i].end.getTime()));
        }
        else {
...
const parseDateString = (dateStr) => {
    ...
    let match = dateStr.match(/^(?:(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\.?\s+)?(\d{4})$/i);
    if (match) {
    ...
    match = dateStr.match(/^(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\.?\s+(\d{4})$/i);
    if (match) {
...
// ============================================================================
const detectSectionPresence = (sections, key) => sections.some((s) => s.key === key);
const isDateOnly = (s) => NORMAL_DATE_RE.test(s.trim());
const isTitleCaseLine = (l) => l.length <= 45 &&
    !/[.!,?;]+$/.test(l) &&
    /^[A-Z][A-Za-z0-9+#.&/:()-]+\s*([A-Z][A-Za-z0-9+#.&/:()-]+\s*)*$/.test(l);
const isProjectNameLine = (l) => isTitleCaseLine(l) &&
...
const isContactLine = (l) => /@/.test(l) ||
    /^\+?\d[\d\s.-]{6,}$/.test(l) ||
    /linkedin|github|\.com|http|www\./i.test(l);
const isLocationLine = (l) => /(dhaka|chittagong|khulna|rajshahi|sylhet|barishal|barisal|rangpur|mymensingh|bangladesh|usa|uk|new york|london|san francisco|toronto|sydney|berlin|india|dubai|california|texas|remote)/i.test(l);
const isEducationLine = (l) => /(university|college|school|institute|bachelor|master|degree|science|arts|engineering|gpa|honours|diploma|b\.sc|m\.sc|ph\.?d|hsc|ssc)/i.test(l);
const isSectionHeading = (l) => {
    const t = l.toLowerCase().trim();
    if (t.length > 40)
        return null;
    if (/^(professional\s+|career\s+|executive\s+)?summary$|^objective$|^about me$/i.test(t))
        return { key: "summary" };
    if (/^(work experience|professional experience|relevant experience|employment history|career history|work history|experience|experience history|career experience)$/i.test(t))
        return { key: "experience" };
    if (/^(technical skills|core competencies|core skills|key skills|skill set|technologies|tech stack|areas of expertise|soft skills|skills|professional skills)$/i.test(t))
        return { key: "skills" };
    if (/^(education|academic background|academic qualifications|educational background|qualifications)$/i.test(t))
        return { key: "education" };
    if (/^(projects|personal projects|key projects|academic projects|project experience|featured projects)$/i.test(t))
        return { key: "projects" };
    if (/^(certifications?|licenses?|licenses & certifications|licenses and certifications|professional certifications|courses|training)$/i.test(t))
        return { key: "certifications" };
    return null;
};
const isSummarySentence = (l) => {
    if (l.length < 40)
        return false;
    if (/\b(?:responsible|passionate|motivated|graduate|professional|developer|engineer|experience)\b/i.test(l))
        return true;
    return false;
};
const pushBucket = (seg, key, line) => {
    seg[key].push(line);
};
/**
 * Walk resume lines top-to-bottom and bucket each into a section.
 */
export const segmentResume = (lines) => {
    const seg = {
        header: [],
        summary: [],
        experience: [],
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (phase === "summary") {
            ...
            if (DATE_RANGE_RE.test(l) || isContactLine(l) || bullet) {
                phase = "experience";
                if (DATE_RANGE_RE.test(l) || bullet)
                    pushBucket(seg, "experience", l);
                continue;
            }
            pushBucket(seg, "summary", l);
...
// ============================================================================
export const parseResumeByDictionary = (text) => {
    const lines = text
        .split("\n")
        .map((l) => l.trim())
        .filter(Boolean);
    ...
    const address = segmented.header
        ...
        .find((l) => /(Dhaka|Chittagong|Khulna|Rajshahi|Sylhet|Barishal|Barisal|Rangpur|Mymensingh|Bangladesh|New York|London|San Francisco|Toronto|Sydney|Berlin|India|USA|UK|Dubai|California|Texas)/i.test(l)) ?? "";
    // Job title: try header, then summary.
    const headerTitleLine = segmented.header
        .slice(1)
        .find((l) => !isContactLine(l) && !isLocationLine(l) && isTitleCaseLine(l)) || "";
    let jobTitle = headerTitleLine ||
    ...
    if (!hasSkillsSection) {
        hardSkills = hardSkills.filter((s) => !HARD_SKILL_STOPWORDS.has(s.toLowerCase()));
    }
...
const parseExperience = (lines) => {
    ...
    for (const rawLine of lines) {
        ...
        if (!isBullet && isDateOnly(line)) {
            ...
            const start = parts[0] || "";
            let end = parts[1] || "";
            if (!end && /(present|current|now|ongoing)/i.test(line))
                end = parts[0];
            if (current) {
                current.startDate = current.startDate || start;
                current.endDate = current.endDate || end;
        ...
        if (header && !isBullet) {
            ...
            startNew(header.role, header.company, header.startDate || pendingDates?.start || "", header.endDate || pendingDates?.end || "");
            continue;
        }
        // If we have an active entry and this isn't a bullet, treat as continuation of last bullet or role info.
        if (!current) {
...
const parseRoleHeader = (line) => {
    ...
    if (/\.$/.test(cleaned))
        return null;
    if (/^(developed|designed|built|implemented|created|managed|led|worked|collaborated|delivered|improved|optimized|reduced|maintained|tested|wrote|architected|launched|owned|handled|assisted|spearheaded|responsible for|contributed|supported|helped|applied)\b/i.test(cleaned))
        return null;
    ...
    const fullRangeMatch = cleaned.match(/((?:\d{1,2}[\/-]\d{1,2}[\/-]\d{2,4}|\d{4}|(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\.?\s*\d{0,4}))\s*[-–—]\s*((?:\d{1,2}[\/-]\d{1,2}[\/-]\d{2,4}|\d{4}|(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\.?\s*\d{0,4}|present|current|now|ongoing))/i);
    let body = cleaned;
    if (fullRangeMatch) {
        result.startDate = fullRangeMatch[1];
        result.endDate = fullRangeMatch[2];
        body = (cleaned.slice(0, fullRangeMatch.index) +
            cleaned.slice(fullRangeMatch.index + fullRangeMatch[0].length)).trim();
    }
    else {
        // Fallback: split date range off the end.
        const dateMatch = cleaned.match(/\s+[-–—|]\s+((?:\d{1,2}[\/-]\d{1,2}[\/-]\d{2,4}|\d{4}|(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\.?\s*\d{0,4}|present|current|now|ongoing))\s*$/i);
        if (dateMatch) {
...
const parseProjects = (lines) => {
    ...
    const isDateRangeLine = (l) => {
        const spaced = l.replace(/([a-zA-Z])(Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:tember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)/g, "$1 $2");
    ...
    for (let idx = 0; idx < lines.length; idx++) {
        ...
        const spaced = line.replace(/([a-zA-Z])(Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:tember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)/g, "$1 $2");
        ...
        const startsNewProject = !current ||
...
// ============================================================================
const isDescriptionLine = (l) => /^(developed|designed|built|implemented|created|used|built with|technologies|features|role|responsibilities)/i.test(l) || l.length > 60;
...
const isLinkLabelLine = (l) => {
    const trimmed = l.trim();
    if (!trimmed)
        return false;
    if (/^(?:https?:\/\/|www\.)\S+/i.test(trimmed))
        return true;
    return /^(?:live\s+(?:link|demo|preview|url|site|app)|live|preview|demo|link|website|site|url|repo|repository|source\s+code|code|github|gitlab|deployment|deployed\s+link|video|youtube|app\s+store|play\s+store)\s*:?$/i.test(trimmed);
};
// ============================================================================
// Education Parsing
// ============================================================================
...
const parseEducation = (lines) => {
    ...
    const flushPending = () => {
        ...
        const degree = matchDictionary(combined, DEGREE_KEYWORDS).find((d) => d.toLowerCase() !== "certification") || "";
    ...
    const pendingHasDegree = () => {
        if (pendingLines.length === 0)
            return false;
        const combined = pendingLines.join(" ");
        return !!matchDictionary(combined, DEGREE_KEYWORDS).find((d) => d.toLowerCase() !== "certification");
    };
    // Check if pending group already has a date line (meaning a previous entry exists).
    const pendingHasDate = () => {
        if (pendingLines.length === 0)
            return false;
        return pendingLines.some((l) => /^\d{4}\s*[-–—]\s*\d{4}$/i.test(l) || DATE_RANGE_RE.test(l));
    };
    for (const raw of lines) {
        ...
        const degree = matchDictionary(line, DEGREE_KEYWORDS).find((d) => d.toLowerCase() !== "certification") || "";
...
// ============================================================================
const detectDateFormatting = (lines) => {
    const text = lines.join("\n");
    const dateMatches = text.match(/\b((?:\d{1,2}[\/-]\d{2,4})|(?:(?:Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:tember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)\.?\s*\d{4})|(?:present|current))\b/gi);
    if (!dateMatches || dateMatches.length === 0)
        return true;
    return dateMatches.every((d) => /present|current/i.test(d) ||
        /^\d{1,2}[\/-]\d{2,4}$/.test(d) ||
        /^(?:Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:tember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)\.?\s*\d{4}$/i.test(d));
};
const inferTone = (text, measurableCount) => {
    const wordCount = countWords(text);
    if (measurableCount >= 3)
        return "good";
    if (measurableCount >= 1)
        return "professional";
    if (wordCount < 100)
        return "weak";
    return "bad";
