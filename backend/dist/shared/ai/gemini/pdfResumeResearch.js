...
export const RESUME_RESEARCH_TEMPLATE = {
    personal_info: {
        fullName: "",
        jobTitle: "",
        contact: {
 ...
 * Resume research result
 ----------------------------------------------------------------*/
export const researchResume = async (resumeText, fileBase64, mimeType) => {
    const parts = [];
 ...
 * Normalize resume research result
 ----------------------------------------------------------------*/
const normalizeResearchResult = (raw) => {
    ...
    return {
        ...
        experience: arr(raw?.experience).map((exp) => ({
            role: str(exp?.role),
        ...
        education: arr(raw?.education).map((edu) => ({
        ...
        skills: {
            ...
            // ),
            hardSkills: arr(raw?.skills?.hardSkills).map((v) => str(v)),
            softSkills: arr(raw?.skills?.softSkills).map((v) => str(v)),
        },
        projects: arr(raw?.projects).map((proj) => ({
            name: str(proj?.name),
