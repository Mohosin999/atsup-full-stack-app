...
import { toResumeText, buildMatchCategory, educationScore, parseYearsOfExperience, countMeasurableResults, measurableResultsScore, summaryScore as summaryWordsScore, getSkillVariants, countVariantsInText, countActionVerbs, actionVerbsScore, } from "./utils";
...
export const calculateLocalMatchScore = (resume, structuredJD) => {
    ...
    if (jd) {
        ...
        softSkillsMatch = buildMatchCategory(resumeText, jd.skills.softSkills, isSkillPresent);
        matchBreakdown = {
        ...
        if (hardSkillsMatch.missing.length)
            suggestions.push("Add missing required hard skills of your resume to better match this job description");
        if (softSkillsMatch.missing.length)
            suggestions.push("Add missing required soft skills of your resume to better match this job description");
        if (jd.experienceYearsRequired > 0 &&
            resumeYears < jd.experienceYearsRequired)
            suggestions.push(`Job requires ${jd.experienceYearsRequired}+ years, but your resume shows ${resumeYears} ${resumeYears === 1 ? "year" : "years"}.`);
    }
    ...
    if (measurable.count < 3)
        suggestions.push(`Add at least ${3 - measurable.count} more measurable results.`);
    if (actionVerbs.count < 3)
        suggestions.push("Use strong action verbs in your experience bullet points (e.g. built, launched, optimized).");
    ...
    const atsFriendliness = Math.round(structureFactors.filter(Boolean).length * 20);
    // =========================================================
    // CATEGORY SCORES
    ...
    const categories = {
        ...
        recruiterTips: buildRecruiterTips(resume, jd, resumeYears, measurable, actionVerbs),
    ...
    // =============================================
    const overallScore = Math.round((categories.searchability.score * CATEGORY_WEIGHTS.searchability +
    ...
    // =============================================
    const experienceSectionScore = experienceCount >= 2
    ...
    const skillSectionScore = jd
        ? Math.round((categories.hardSkills.score + categories.softSkills.score) / 2)
