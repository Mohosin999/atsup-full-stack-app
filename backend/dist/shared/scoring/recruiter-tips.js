...
import { scoreFromChecks, scoreFromSubgroups, deriveFeedback, toResumeText, actionVerbsScore, measurableResultsScore, summaryScore, } from "./utils";
// ============================================================
// Summary
// ============================================================
const buildSummarySubgroup = (resume) => {
    const summaryWords = (resume.summary || "")
        .split(/\s+/)
        .filter(Boolean).length;
    const score = summaryScore(summaryWords);
    const status = score >= 100 ? "passed" : "failed";
    const detail = summaryWords >= 30 && summaryWords <= 80
...
// ============================================================
const buildJobLevelSubgroup = (jd, resumeYears) => {
    let status;
    let detail;
...
const buildMeasurableSubgroup = (measurable) => {
    ...
    const detail = measurable.count >= 3
...
const buildActionVerbsSubgroup = (actionVerbs) => {
    ...
    const status = score >= 60 ? "passed" : "failed";
    const detail = actionVerbs.count >= 3
...
// Word count
// ============================================================
const countResumeWords = (resume) => {
    if (resume.wordCount)
        return Number(resume.wordCount) || 0;
    return toResumeText(resume).split(/\s+/).filter(Boolean).length;
};
const buildWordCountSubgroup = (wordCount) => {
    let status;
    let detail;
...
// ============================================================
export const buildRecruiterTips = (resume, jd, resumeYears, measurable, actionVerbs) => {
