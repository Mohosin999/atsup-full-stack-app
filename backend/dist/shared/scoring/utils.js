import { extractSkillsFromResume, getSkillVariants, countVariantsInText, } from "./keywords";
import { MEASURABLE_RESULT_RE, ACTION_VERBS } from "./constants";
export const toResumeText = (resume) => {
    const parts = [];
...
export const buildMatchCategory = (resumeText, items, isPresent) => {
    if (!items?.length) {
    ...
    const score = Math.round(results.reduce((sum, r) => sum + r.itemScore, 0) / results.length);
    return {
        score,
...
export const scoreFromChecks = (checks) => {
    ...
    for (const c of checks) {
        if (c.status === "not-applicable" || c.weight <= 0)
            continue;
        total += c.weight;
        if (c.status === "passed")
            earned += c.weight;
...
export const scoreFromSubgroups = (subgroups) => {
    let earned = 0, total = 0;
...
export const deriveFeedback = (checks) => {
    const strengths = [];
    const improvements = [];
    for (const c of checks) {
        if (c.status === "not-applicable" || c.weight <= 0)
            continue;
        if (c.status === "passed")
            strengths.push(c.detail);
        else
            improvements.push(`${c.label}: ${c.detail}`);
    }
...
// ============================================================
export const educationScore = (resume, jdEducation) => {
    ...
    const resumeRanks = resumeEdu.map((edu) => getRank(`${edu.degree} ${edu.field} ${edu.education_level}`));
    ...
    if (resumeMaxRank === 0)
        return 45;
    if (resumeMaxRank === jdRank)
        return 100;
    if (resumeMaxRank > jdRank)
        return 95;
    if (resumeMaxRank === jdRank - 1)
        return 55;
    return 30;
};
// ============================================================
...
// ============================================================
export const parseYearsOfExperience = (raw) => {
    if (raw == null || raw === "")
        return 0;
...
export const countMeasurableResults = (resume) => {
...
export const measurableResultsScore = (count) => count >= 3 ? 100 : count === 2 ? 80 : count === 1 ? 60 : 0;
const ACTION_VERBS_RE = new RegExp(`\\b(?:${ACTION_VERBS.map((v) => v.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")).join("|")})\\b`, "gi");
const countActionVerbs = (resume) => {
    const highlights = (resume.experience || []).flatMap((exp) => exp.responsibilities || []);
...
const actionVerbsScore = (count) => count >= 3 ? 100 : count === 2 ? 80 : count === 1 ? 60 : 0;
export const summaryScore = (summaryWords) => summaryWords >= 30 && summaryWords <= 80
    ? 100
    : summaryWords >= 80
        ? 60
        : summaryWords >= 10
...
// 
// ============================================================
export const collectResumeDates = (resume) => {
    const dates = [];
    const push = (raw) => {
        if (!raw)
            return;
...
export { extractSkillsFromResume, getSkillVariants, countVariantsInText, countActionVerbs, actionVerbsScore, };
