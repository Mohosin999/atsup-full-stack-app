...
export const extractPortfolio = (text: string): string => {
  ...
  if (
    /(linkedin|github|gitlab|behance|dribbble|twitter|facebook|fb\.)/i.test(
      candidate,
    )
  ) {
    return "";
  ...
  if (
    emailDomain &&
    bareCandidate.toLowerCase().endsWith(emailDomain.toLowerCase())
  ) {
    return "";
  }
...
const EXPERIENCE_DURATION_RE =
  /\b\d+(?:\.\d+)?\s*\+?\s*(?:years?|yrs?)\s+of\s+experience\b/i;
