...
interface SkillGroup {
  /** The single canonical name used for display. */
  display: string;
  /** Alternative spellings (any case) that should collapse to `display`. */
  aliases: string[];
}
...
const LOOKUP: Record<string, SkillGroup> = {};
...
const GROUP_PATTERNS: Array<{ display: string; re: RegExp }> = SKILL_GROUPS.map(
  (group) => {
 ...
 */
function extractEmbeddedSkill(token: string): string | null {
 ...
 */
export const canonicalizeSkill = (raw: string): string => {
 ...
 */
export const normalizeHardSkills = (skills: string[]): string[] => {
  ...
  const add = (skill: string) => {
    ...
    if (!canonical) return;
    if (SKILL_STOPWORDS.has(canonical.toLowerCase())) return;
  ...
  for (const raw of skills) {
    ...
    if (LOOKUP[normalizeKey(raw)]) {
      ...
      for (const part of splitEntry(raw)) add(part);
    }
  }
 ...
 */
export const getSkillAliases = (skill: string): string[] => {
