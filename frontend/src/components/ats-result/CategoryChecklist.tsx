...
const STATUS_ICON: Record<CheckStatus, { icon: React.ReactNode; color: string }> = {
  passed: { icon: <CheckCircle2 className="w-4 h-4" />, color: "text-green-600" },
  partial: { icon: <AlertCircle className="w-4 h-4" />, color: "text-amber-600" },
...
const SubgroupList: React.FC<{
  ...
  <div className="rounded-lg border border-gray-200/40 bg-gray-100 overflow-hidden">
    <div className="flex items-center justify-between gap-2 px-3 py-1.5 bg-gray-50 border-b border-gray-200/40">
...
const CategoryRow: React.FC<{
  ...
  return (
    <div className="rounded-xl border border-gray-200/60 bg-gray-100 overflow-hidden">
      <button
        onClick={() => setOpen((v) => !v)}
        className="w-full flex items-center gap-3 px-3.5 py-3 text-left hover:bg-white/90 transition-colors"
      >
...
const CategoryChecklist: React.FC<CategoryChecklistProps> = ({
  ...
  return (
    <div className="bg-white/80 rounded-2xl border border-gray-200/60 p-5 backdrop-blur-sm">
