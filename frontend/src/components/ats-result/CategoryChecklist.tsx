...
const CategoryRow: React.FC<{
  ...
  hasFormattingData?: boolean;
  onShowTips?: () => void;
}> = ({ category, isFormatting, hasFormattingData }) => {
  const activeChecks = category.checks.filter(
    (c) => c.status !== "not-applicable",
  );
  const passedCount = activeChecks.filter((c) => c.status === "passed").length;
  ...
  const totalItems = hasMatchData
    ? (category.matched?.length ?? 0) + (category.missing?.length ?? 0)
    : activeChecks.length;
  const isLow = category.score < 50;
  const barColor = isLow ? "bg-red-500" : "bg-stone-900 dark:bg-lime-300";
  ...
  if (isFormatting && !hasFormattingData) {
    return (
      <div className="overflow-hidden relative">
        <div className="w-full flex items-center gap-3 px-3.5 py-1.5 text-left">
          <div className="flex-1 min-w-0 blur-[0.1px] select-none pointer-events-none opacity-40">
            <div className="flex items-center justify-between gap-2">
              <span className="font-plex text-xs text-stone-800 dark:text-stone-200 truncate">
                {category.title}
              </span>
              <span className="font-plex text-xs text-stone-800 dark:text-stone-100 truncate">
                {totalItems - matchedCount} issues to fix
              </span>
            </div>
            <div className="mt-1.5 flex items-center gap-2">
              <div className="flex-1 h-1.5 bg-stone-200 dark:bg-stone-700 overflow-hidden rounded-full">
                <motion.div
                  className="h-full bg-stone-900 dark:bg-lime-300"
                  initial={{ width: 0 }}
                  animate={{ width: `${category.score}%` }}
                  transition={{ duration: 0.8, ease: "easeOut" }}
                />
  ...
  return (
    <div className="overflow-hidden">
      <div className="w-full flex items-center gap-3 px-3.5 py-1.5 text-left rounded-lg hover:bg-stone-50 dark:hover:bg-stone-800/60 transition-colors">
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2">
            <span className="font-plex text-xs text-stone-800 dark:text-stone-200 truncate">
              {category.title}
            </span>
            <span className={`font-plex text-xs ${labelColor} truncate`}>
              {totalItems - matchedCount} issues to fix
            </span>
          </div>
          <div className="mt-1.5 flex items-center gap-2">
...
const CategoryChecklist: React.FC<CategoryChecklistProps> = ({
  ...
  return (
    <div className="px-5 py-7 bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 rounded-2xl">
      <div className="flex justify-center">
        <ScoreCircle score={overallScore} size="md" />
      </div>
      ...
      {onRescan && (
        <div className="flex pt-6 pb-8 w-full px-3.5">
          <button
            onClick={onRescan}
            className="font-plex w-full inline-flex items-center justify-center gap-1.5 px-4 py-2.5 text-sm font-medium bg-stone-900 dark:bg-lime-300 text-stone-50 dark:text-stone-900 hover:bg-stone-800 dark:hover:bg-lime-200 rounded-lg transition-colors"
          >
            <RefreshCw className="w-4 h-4" />
            Upload & rescan
          </button>
