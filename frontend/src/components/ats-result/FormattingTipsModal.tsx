...
//             {/* Layout tips */}
//               <div>
//                   <p className="flex items-center gap-2 text-sm font-semibold text-gray-800 dark:text-gray-100 mb-3">
...
//                     <li
//                       key={tip}
//                       className="flex items-center gap-2.5 text-[13px] lg:text-sm text-gray-700 dark:text-gray-300 leading-snug"
//                     >
//                       <span className="ml-5 w-1.5 h-1.5 bg-gray-500 flex-shrink-0" />
//                       {tip}
//                     </li>
...
import { motion, AnimatePresence } from "framer-motion";
import { X, LayoutTemplate, Type } from "lucide-react";
      ...
      {isOpen && (
        ...
        >
          <motion.div
            ...
            animate={{ scale: 1, opacity: 1 }}
            exit={{ scale: 0.95, opacity: 0 }}
            className="bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 rounded-2xl shadow-xl w-full max-w-2xl max-h-[85vh] flex flex-col overflow-hidden"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-start justify-between gap-4 px-6 py-5 border-b border-stone-100 dark:border-stone-800">
              <div>
                <h3 className="font-fraunces text-lg md:text-xl text-stone-900 dark:text-stone-50 leading-tight">
                  Formatting tips
                </h3>
                <p className="font-plex text-sm text-stone-500 dark:text-stone-400 mt-0.5">
                  Follow these free tips to make your resume perfectly
                  ATS-friendly.
                </p>
              </div>
              <button
                onClick={onClose}
                className="p-2 text-stone-400 dark:text-stone-500 hover:text-stone-700 dark:hover:text-stone-300 hover:bg-stone-100 dark:hover:bg-stone-800 transition-colors flex-shrink-0 rounded-full"
              >
                <X className="w-5 h-5" />
              </button>
            </div>

            <div className="font-plex flex-1 overflow-y-auto px-6 py-5 space-y-6">
              {/* Layout tips */}
              <div>
                <p className="flex items-center gap-2 text-sm font-semibold text-stone-800 dark:text-stone-100 mb-3">
                  <LayoutTemplate className="w-4 h-4 text-stone-500 dark:text-lime-300" />
                  Layout tips
                </p>
                <ul className="space-y-2.5">
                  {LAYOUT_TIPS.map((tip) => (
                    <li
                      key={tip}
                      className="flex items-start gap-2.5 text-[13px] lg:text-sm text-stone-600 dark:text-stone-300 leading-snug"
                    >
                      <span className="mt-1.5 w-1.5 h-1.5 rounded-full bg-lime-400 dark:bg-lime-300 flex-shrink-0" />
                      {tip}
                    </li>
                  ))}
                </ul>
              ...
              <div>
                <p className="flex items-center gap-2 text-sm font-semibold text-stone-800 dark:text-stone-100 mb-3">
                  <Type className="w-4 h-4 text-stone-500 dark:text-lime-300" />
                  Font tips
                </p>
                <ul className="space-y-2.5">
                  {FONT_TIPS.map((tip) => (
