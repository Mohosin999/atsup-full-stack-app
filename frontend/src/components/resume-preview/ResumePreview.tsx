...
const formatDescription = (
  ...
  return (
    <ul
      className={`list-outside list-disc pl-4 space-y-0.5 text-sm ${forPdf ? "text-gray-800" : "text-gray-800"}`}
    >
      {lines.map((line, i) => {
...
export default function ResumePreview({
  ...
  const bgColor = forPdf ? "bg-white" : "bg-white";
  const textColor = forPdf
    ? "text-gray-800"
    : "text-gray-800 text-sm font-sans";
